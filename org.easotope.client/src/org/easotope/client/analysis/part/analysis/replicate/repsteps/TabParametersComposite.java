...
public class TabParametersComposite extends EasotopeComposite {
	...
	private RepStepParamComposite repStepParamComposite;
	...
	public TabParametersComposite(ChainedPart chainedPart, Composite parent, int style) {
		...
		instructions.setText(Messages.repTabParametersComposite_instructions);
	...
	public void setCompositeClass(String parameterComposite) {
		if (parameterComposite == null) {
			if (repStepParamComposite != null) {
				repStepParamComposite.dispose();
				repStepParamComposite = null;
			}
		...
		if (repStepParamComposite != null && !repStepParamComposite.getClass().getName().equals(parameterComposite)) {
			repStepParamComposite.dispose();
			repStepParamComposite = null;
		}
		...
		repStepParamComposite = (RepStepParamComposite) Reflection.createObject(parameterComposite, (ChainedPart) getEasotopePart(), fullComposite, SWT.NONE);
		FormData formData = new FormData();
		formData.top = new FormAttachment(instructions, 5);
		formData.left = new FormAttachment(0);
		formData.right = new FormAttachment(100);
		formData.bottom = new FormAttachment(100);
		repStepParamComposite.setLayoutData(formData);
