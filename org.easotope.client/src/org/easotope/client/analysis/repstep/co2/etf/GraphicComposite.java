...
import org.easotope.shared.analysis.repstep.co2.etf.Calculator.GraphPoint;
...
public class GraphicComposite extends RepStepGraphicComposite {
	...
	public void newCalculation(ScratchPad<ReplicatePad> scratchPad) {
		...
		ArrayList<GraphPoint> standardGraphPoints = (ArrayList<GraphPoint>) replicatePad.getVolatileData(Calculator.getVolatileDataStandardGraphPointsKey());
		@SuppressWarnings("unchecked")
		ArrayList<ConnectingLine> connectingLines = (ArrayList<ConnectingLine>) replicatePad.getVolatileData(Calculator.getVolatileDataConnectingLines());
		@SuppressWarnings("unchecked")
		...
		for (GraphPoint graphPoint : standardGraphPoints) {
			...
			if (graphPoint.getDisabled()) {
				...
				tooltip = new String[] { graphPoint.getName() + " " + Messages.co2EtfPbl_disabled, timestamp, Messages.co2EtfPbl_d47 + String.valueOf(graphPoint.getX()), Messages.co2EtfPbl_D47 + String.valueOf(graphPoint.getY()) };
				influencesAutoscale = false;
			} else {
				color = ColorCache.getColorFromPalette(getDisplay(), graphPoint.getColorId());
				tooltip = new String[] { graphPoint.getName(), timestamp, Messages.co2EtfPbl_d47 + String.valueOf(graphPoint.getX()), Messages.co2EtfPbl_D47 + String.valueOf(graphPoint.getY()) };
			}
		...
		for (GraphPoint etfPoint : etfGraphPoints) {
			...
			if (name == null) {
				...
				if (etfPoint.getDisabled()) {
					...
					tooltip = new String[] { etfPoint.getName() + " " + Messages.co2EtfPbl_disabled, timestamp, Messages.co2EtfPbl_D47 + String.valueOf(etfPoint.getX()), Messages.co2EtfPbl_D47CDES + String.valueOf(etfPoint.getY()) };
					influencesAutoscale = false;
				} else {
					color = ColorCache.getColorFromPalette(getDisplay(), etfPoint.getColorId());

					if (timestamp != null) {
						tooltip = new String[] { etfPoint.getName(), timestamp, Messages.co2EtfPbl_D47 + String.valueOf(etfPoint.getX()), Messages.co2EtfPbl_D47CDES + String.valueOf(etfPoint.getY()) };
					} else {
						tooltip = new String[] { etfPoint.getName(), Messages.co2EtfPbl_D47 + String.valueOf(etfPoint.getX()), Messages.co2EtfPbl_D47CDES + String.valueOf(etfPoint.getY()) };
					}
				}
