...
import org.easotope.shared.analysis.repstep.superclass.drift.Calculator;
...
public class ParameterComposite extends RepStepParamComposite implements StandardCacheStandardListGetListener {
	...
	private VButton applyStretching;
	...
	public ParameterComposite(ChainedPart chainedPart, Composite parent, int style) {
		...
		windowTypeLabel.setText(Messages.driftParameterComposite_windowType);
		...
		possibilities.put(WindowType.CorrInterval.ordinal(), Messages.driftParameterComposite_corrInterval);
		possibilities.put(WindowType.Window.ordinal(), Messages.driftParameterComposite_window);
		windowType.setPossibilities(possibilities);

		Label minNumStandardsBeforeAndAfterLabel = new Label(this, SWT.NONE);
		minNumStandardsBeforeAndAfterLabel.setText(Messages.driftParameterComposite_minNumStandardsBeforeAndAfter);
		...
		standardsLabel.setText(Messages.driftParameterComposite_standards);
		...
		Label applyStretchingLabel = new Label(this, SWT.NONE);
		applyStretchingLabel.setText(Messages.driftParameterComposite_applyStretching);

		applyStretching = new VButton(this, SWT.CHECK);
		applyStretching.addSelectionListener(new LoggingSelectionAdaptor() {
			public void loggingWidgetSelected(SelectionEvent e) {
				widgetStatusChanged();
			}
		});
	...
	protected HashMap<String,Object> buildNewParameters() {
		...
		parameters.put(Calculator.PARAMETER_APPLY_STRETCHING, applyStretching.getSelection());
	...
	protected void setCurrentFieldValues() {
		...
		Boolean bool = (Boolean) getParameter(Calculator.PARAMETER_APPLY_STRETCHING);
		bool = (bool == null) ? Calculator.DEFAULT_APPLY_STRETCHING : bool;

		applyStretching.setSelection(bool);
	}
	...
	public void enableWidgets() {
		...
		applyStretching.setEnabled(hasAdminPermissions);

		if (!hasAdminPermissions) {
			minNumStandardsBeforeAfter.revert();
			standards.revert();
			applyStretching.revert();
		}
	}
	...
	public void disableWidgets() {
		...
		applyStretching.setEnabled(false);
	}
	...
	protected boolean isDirty() {
		...
		isDirty = isDirty || applyStretching.hasChanged();
