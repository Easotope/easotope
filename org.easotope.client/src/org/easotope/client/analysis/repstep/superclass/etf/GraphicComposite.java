...
package org.easotope.client.analysis.repstep.superclass.etf;
...
import org.easotope.shared.analysis.repstep.superclass.etf.Calculator.AverageLine;
import org.easotope.shared.analysis.repstep.superclass.etf.Calculator.ConnectingLine;
import org.easotope.shared.analysis.repstep.superclass.etf.Calculator.GraphPoint;
import org.easotope.shared.analysis.repstep.superclass.etf.VolatileKeys;
...
public abstract class GraphicComposite extends RepStepGraphicComposite {
	...
	public abstract String getGraph1VerticalLabel();
	public abstract String getGraph1HorizontalLabel();
	public abstract String getGraph2VerticalLabel();
	public abstract String getGraph2HorizontalLabel();
	public abstract String getCorrectorLabel();
	public abstract String getCorrecteeLabel();
	public abstract String getCorrecteePostLabel();
	...
	public GraphicComposite(EasotopePart easotopePart, Composite parent, int style) {
		...
		graph1.setVerticalAxisLabel(getGraph1VerticalLabel());
		graph1.setVerticalAxisShowLabel(true);
		graph1.setVerticalAxisShowValues(true);
		graph1.setHorizontalAxisLabel(getGraph1HorizontalLabel());
		...
		graph2.setVerticalAxisLabel(getGraph2VerticalLabel());
		graph2.setVerticalAxisShowLabel(true);
		graph2.setVerticalAxisShowValues(true);
		graph2.setHorizontalAxisLabel(getGraph2HorizontalLabel());
	...
	public void newCalculation(ScratchPad<ReplicatePad> scratchPad) {
		...
		ArrayList<GraphPoint> standardGraphPoints = (ArrayList<GraphPoint>) replicatePad.getVolatileData(getVolatileKeys().getVolatileDataStandardGraphPointsKey());
		@SuppressWarnings("unchecked")
		ArrayList<ConnectingLine> connectingLines = (ArrayList<ConnectingLine>) replicatePad.getVolatileData(getVolatileKeys().getVolatileDataConnectingLines());
		@SuppressWarnings("unchecked")
		HashSet<AverageLine> averageLines = (HashSet<AverageLine>) replicatePad.getVolatileData(getVolatileKeys().getVolatileDataAverageLinesKey());
		@SuppressWarnings("unchecked")
		HashSet<GraphPoint> etfGraphPoints = (HashSet<GraphPoint>) replicatePad.getVolatileData(getVolatileKeys().getVolatileDataEtfGraphPointsKey());
		LinearRegression etfRegression = (LinearRegression) replicatePad.getVolatileData(getVolatileKeys().getVolatileDataEtfRegressionKey()); 
		...
		for (GraphPoint graphPoint : standardGraphPoints) {
			...
			if (graphPoint.getDisabled()) {
				...
				tooltip = new String[] { graphPoint.getName() + " " + Messages.co2EtfPbl_disabled, timestamp, getCorrectorLabel() + String.valueOf(graphPoint.getX()), getCorrecteeLabel() + String.valueOf(graphPoint.getY()) };
				influencesAutoscale = false;
			} else {
				color = ColorCache.getColorFromPalette(getDisplay(), graphPoint.getColorId());
				tooltip = new String[] { graphPoint.getName(), timestamp, getCorrectorLabel() + String.valueOf(graphPoint.getX()), getCorrecteeLabel() + String.valueOf(graphPoint.getY()) };
			}
		...
		for (ConnectingLine connectingLine : connectingLines) {
			...
			if (connectingLine.getDisabled()) {
				color = ColorCache.getFadedColorFromPalette(getDisplay(), connectingLine.getColorId());
			} else {
				color = ColorCache.getColorFromPalette(getDisplay(), connectingLine.getColorId());
			}

			LineWithEnds lineWithEnds = new LineWithEnds(connectingLine.getX1(), connectingLine.getY1(), connectingLine.getX2(), connectingLine.getY2(), color, null);
			lineWithEnds.setInfluencesAutoscale(!connectingLine.getDisabled());
			graph1.addDrawableObjectFirst(lineWithEnds);
		}
		...
		if (averageLines != null) {  
			for (AverageLine averageLine : averageLines) {
				...
				String[] tooltip = new String[] { Messages.co2EtfPbl_intercept + String.valueOf(intercept) };
		...
		for (GraphPoint etfPoint : etfGraphPoints) {
			...
			if (name == null) {
				...
				tooltip = new String[] { Messages.co2EtfPbl_sample, getCorrecteeLabel() + String.valueOf(etfPoint.getX()), getCorrecteePostLabel() + String.valueOf(etfPoint.getY()) };
				...
				if (etfPoint.getDisabled()) {
					...
					tooltip = new String[] { etfPoint.getName() + " " + Messages.co2EtfPbl_disabled, timestamp, getCorrecteeLabel() + String.valueOf(etfPoint.getX()), getCorrecteePostLabel() + String.valueOf(etfPoint.getY()) };
					influencesAutoscale = false;
				} else {
					color = ColorCache.getColorFromPalette(getDisplay(), etfPoint.getColorId());

					if (timestamp != null) {
						tooltip = new String[] { etfPoint.getName(), timestamp, getCorrecteeLabel() + String.valueOf(etfPoint.getX()), getCorrecteePostLabel() + String.valueOf(etfPoint.getY()) };
					} else {
						tooltip = new String[] { etfPoint.getName(), getCorrecteeLabel() + String.valueOf(etfPoint.getX()), getCorrecteePostLabel() + String.valueOf(etfPoint.getY()) };
					}
				}
		...
		if (etfRegression != null) {
			...
			point.setTooltip(new String[] { Messages.co2EtfPbl_intercept + String.valueOf(etfRegression.getIntercept()) });
			graph2.addDrawableObjectFirst(point);

			double xIntercept = (0 - etfRegression.getIntercept()) / etfRegression.getSlope();
			point = new Point(xIntercept, 0, null);
			point.setTooltip(new String[] { Messages.co2EtfPbl_intercept + String.valueOf(xIntercept) });
			graph2.addDrawableObjectFirst(point);
		}
