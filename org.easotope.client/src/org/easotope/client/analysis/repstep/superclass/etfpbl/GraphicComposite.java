...
package org.easotope.client.analysis.repstep.superclass.etfpbl;
...
import org.easotope.shared.analysis.repstep.superclass.etfpbl.Calculator.AverageLine;
import org.easotope.shared.analysis.repstep.superclass.etfpbl.Calculator.GraphPoint;
...
public abstract class GraphicComposite extends RepStepGraphicComposite {
	...
	public void newCalculation(ScratchPad<ReplicatePad> scratchPad) {
		...
		HashSet<GraphPoint> standardGraphPoints = (HashSet<GraphPoint>) replicatePad.getVolatileData(getVolatileKeys().getVolatileDataStandardGraphPointsKey());
		@SuppressWarnings("unchecked")
		...
		for (GraphPoint etfPoint : etfGraphPoints) {
			...
			if (etfPoint.getName() == null) {
				...
				if (etfPoint.getDisabled()) {
					...
					tooltip = new String[] { etfPoint.getName() + " " + Messages.co2Etf_disabled, timestamp, getCorrecteeLabel() + String.valueOf(etfPoint.getX()), getCorrecteePostLabel() + String.valueOf(etfPoint.getY()) };
					influencesAutoscale = false;
				} else {
