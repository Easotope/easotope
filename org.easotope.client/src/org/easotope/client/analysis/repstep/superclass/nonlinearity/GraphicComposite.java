...
package org.easotope.client.analysis.repstep.superclass.nonlinearity;
...
import org.easotope.shared.analysis.repstep.superclass.nonlinearity.Calculator.NonlinearityPoint;
...
public abstract class GraphicComposite extends RepStepGraphicComposite {
	...
	public abstract String getCorrecteeMeasuredLabel();
	public abstract String getCorrecteeCorrectedLabel();
	...
	public GraphicComposite(EasotopePart easotopePart, Composite parent, int style) {
		...
		graph.setVerticalAxisLabel(getVerticalLabel());
		graph.setVerticalAxisShowLabel(true);
		graph.setVerticalAxisShowValues(true);
		graph.setHorizontalAxisLabel(getHorizontalLabel());
	...
	public void newCalculation(ScratchPad<ReplicatePad> scratchPad) {
		...
		MultiLineCommonSlopeRegression regression = (MultiLineCommonSlopeRegression) replicatePad.getVolatileData(getVolatileKeys().getVolatileDataLinearRegressionKey());
		...
		Double addedCorrection = (Double) replicatePad.getVolatileData(getVolatileKeys().getVolatileDataNonlinearityCorrectionKey());
		...
		HashSet<NonlinearityPoint> nonlinearityPoints = (HashSet<NonlinearityPoint>) replicatePad.getVolatileData(getVolatileKeys().getVolatileDataNonlinearityPointsKey());
		...
		for (NonlinearityPoint nonlinearityPoint : nonlinearityPoints) {
			...
			double corrector = nonlinearityPoint.getCorrector();
			double correctee = nonlinearityPoint.getCorrectee();
			long date = nonlinearityPoint.getDate();

			if (!nonlinearityPoint.getDisabled()) {
				minX = Math.min(minX, corrector);
				maxX = Math.max(maxX, corrector);
			}
			...
			if (standard != null) {
				...
				if (nonlinearityPoint.getDisabled()) {
					...
					tooltip = new String[] { standard.getName() + " " + Messages.nonlinearityGraphicComposite_disabled, timestamp, getCorrectorLabel() + String.valueOf(corrector), getCorrecteeLabel() + String.valueOf(correctee) };
					influencesAutoscale = false;				
				} else {
					color = ColorCache.getColorFromPalette(getDisplay(), standard.getColorId());
					tooltip = new String[] { standard.getName(), timestamp, getCorrectorLabel() + String.valueOf(corrector), getCorrecteeLabel() + String.valueOf(correctee) };
				}

				pointDesign = new PointDesign(getDisplay(), color, PointStyle.values()[standard.getShapeId()]);
				Point point = new Point(corrector, correctee, pointDesign);
				point.setTooltip(tooltip);
				point.setInfluencesAutoscale(influencesAutoscale);
				graph.addDrawableObjectLast(point);

			} else {
				tooltip = new String[] { timestamp, getCorrectorLabel() + String.valueOf(corrector), getCorrecteeMeasuredLabel() + String.valueOf(correctee-addedCorrection), getCorrecteeCorrectedLabel() + String.valueOf(correctee) };
				pointDesign = new PointDesign(getDisplay(), ColorCache.getColor(getDisplay(), ColorCache.BLACK), PointStyle.X);
				graph.addDrawableObjectFirst(new LineWithEnds(0, correctee, corrector, correctee-addedCorrection, ColorCache.getColor(getDisplay(), ColorCache.BLACK), SWT.LINE_DOT, null));

				Point point = new Point(0, correctee, pointDesign);
				point.setTooltip(tooltip);
				graph.addDrawableObjectLast(point);

				point = new Point(corrector, correctee-addedCorrection, null);
				graph.addDrawableObjectLast(point);
			}
		}
