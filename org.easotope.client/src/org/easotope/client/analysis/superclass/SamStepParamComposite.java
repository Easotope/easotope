...
public abstract class SamStepParamComposite extends EditorComposite implements CorrIntervalCacheCorrIntervalCompGetListener, CorrIntervalCacheRepStepParamsSaveListener {
	...
	protected SamStepParamComposite(ChainedPart chainedPart, Composite parent, int style) {
	...
	public void corrIntervalCompUpdated(int commandId, int corrIntervalId, int dataAnalysisTypeId, List<RepStepParams> repStepParams, CorrIntervalScratchPad corrIntervalScratchPad, List<CorrIntervalError> errors) {
		if (this.corrIntervalId == corrIntervalId && this.repAnalysisId == dataAnalysisTypeId) {
			for (RepStepParams parameter : repStepParams) {
				if (parameter.getPosition() == repStepPosition) {
