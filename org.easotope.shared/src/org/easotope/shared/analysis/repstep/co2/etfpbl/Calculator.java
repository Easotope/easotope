...
package org.easotope.shared.analysis.repstep.co2.etfpbl;
...
import org.easotope.shared.analysis.repstep.co2.etfpbl.dependencies.Dependencies;
...
public class Calculator extends RepStepCalculator {
	...
	public static String getVolatileDataEtfGraphPointsKey() {
		return Calculator.class.getName() + "VOLATILE_DATA_ETF_GRAPH_POINTS";
	}
	...
	public void calculate(ReplicatePad[] replicatePads, int padNumber, DependencyManager dependencyManager) throws EasotopeStepException {
		...
		for (ReplicatePad replicatePad : standardReplicatePads.getUsable()) {
			...
			if (getAverageStandardsFirst()) {
				...
				tempAverageLine.addNumber(standardΔ47);
			}

			if (!Double.isNaN(standardδ47) && !Double.isNaN(standardΔ47)) {
				standardGraphPoints.add(new GraphPoint(sourceId, date, standard.getName(), standardδ47, standardΔ47, colorId, shapeId, false));
			}
		}
		...
		for (ReplicatePad replicatePad : standardReplicatePads.getDisabled()) {
			...
			if (!Double.isNaN(standardδ47) && !Double.isNaN(standardΔ47)) {
				standardGraphPoints.add(new GraphPoint(sourceId, date, standard.getName(), standardδ47, standardΔ47, colorId, shapeId, true));
			}
		}
		...
		if (getAverageStandardsFirst()) {
			replicatePads[padNumber].setVolatileData(getVolatileDataAverageLinesKey(), new HashSet<AverageLine>(standardIdToAverageLines.values()));
		}
		...
		if (getAverageStandardsFirst()) {
			for (int standardId : standardIdToAverageLines.keySet()) {
				...
				if (!standardIdToKnownΔ47NoAcidCorr.containsKey(standardId)) {
					String message = MessageFormat.format(Messages.etfPblCalc_noKnownD47, standard.getName());
	...
	public class GraphPoint {
		...
		public GraphPoint(int standardId, long date, String name, double x, double y, int colorId, int shapeId, boolean isDisabled) {
