...
public class AlphaPlugin extends DependencyPlugin {
	...
	public HashMap<String,String> getPrintableValues(DependencyManager dependencyManager) {
		...
		NumericValue δ18O = (acidTemp == null) ? null : acidTemp.getValues().get(AcidTempParameter.δ18O.ordinal());
		result.put(getName(), (δ18O == null) ? "UNDEFINED" : String.valueOf(δ18O.getValue()));
	...
	public boolean verifyCurrentObject(DependencyManager dependencyManager) {
		...
		if (acidTempId != DatabaseConstants.EMPTY_DB_ID && acidTemp != null && acidTemp.getId() == acidTempId && acidTemp.getValues().get(AcidTempParameter.δ18O.ordinal()) != null) {
			return true;
		}
