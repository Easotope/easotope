...
package org.easotope.shared.analysis.repstep.superclass.acidfrac.dependencies;
...
public abstract class AlphaPlugin extends DependencyPlugin {
	...
	public HashMap<String,String> getPrintableValues(DependencyManager dependencyManager) {
		...
		NumericValue clumped = (acidTemp == null) ? null : acidTemp.getValues().get(getAcidTempParameter().ordinal());
		result.put(getName(), (clumped == null) ? "UNDEFINED" : String.valueOf(clumped.getValue()));
	...
	public boolean verifyCurrentObject(DependencyManager dependencyManager) {
		...
		if (acidTempId != DatabaseConstants.EMPTY_DB_ID && acidTemp != null && acidTemp.getId() == acidTempId && acidTemp.getValues().get(getAcidTempParameter().ordinal()) != null) {
			return true;
		}
