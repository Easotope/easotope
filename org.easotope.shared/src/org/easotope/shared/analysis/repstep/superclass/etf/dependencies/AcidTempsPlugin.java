...
package org.easotope.shared.analysis.repstep.superclass.etf.dependencies;
...
import org.easotope.shared.analysis.repstep.superclass.etf.Calculator;
import org.easotope.shared.core.DoubleTools;
import org.easotope.shared.core.NumericValue;
import org.easotope.shared.core.cache.AbstractCache;

public abstract class AcidTempsPlugin extends DependencyPlugin {
	...
	protected abstract String getMZasString();
	protected abstract StandardParameter getStandardParameter();
	protected abstract AcidTempParameter getAcidTempParameter();

	protected AcidTempsPlugin() {
	...
	public HashMap<String,String> getPrintableValues(DependencyManager dependencyManager) {
		...
		for (int i=0; i<standards.length; i++) {
			...
			result.put(standard.getName() + " " + getMZasString(), (value == null) ? "UNDEFINED" : value);

			AcidTemp acidTemp = acidTemps[i];
			numericValue = acidTemp == null ? null : acidTemp.getValues().get(getAcidTempParameter().ordinal());
			value = null;
			...
			result.put(standard.getName() + " " + calculator.getAcidTemperature() + "° " + getMZasString() + " α", (value == null) ? "UNDEFINED" : value);
		}
	...
	public boolean verifyCurrentObject(DependencyManager dependencyManager) {
		for (int i=0; i<standards.length; i++) {
			...
			if (standard.getValues().get(getStandardParameter().ordinal()) == null) {
				return false;
			}
			...
			if (sampleType.getHasAcidTemps()) {	
				...
				if (acidTemp.getValues().get(getAcidTempParameter().ordinal()) == null) {
					return false;
				}
			} else {
				if (acidTemp != null) {
