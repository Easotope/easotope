...
public class UserListGet extends Command {
	...
	public void execute(ConnectionSource connectionSource, RawFileManager rawFileManager, Hashtable<String,Object> authenticationObjects) throws Exception {
		...
		GenericRawResults<User> rawResults = userDao.queryRaw(
			"select id,username from " + User.TABLE_NAME + " WHERE " + 
		...
		for (User user : rawResults) {
			userList.put(user.getId(), new UserListItem(user.getUsername(), true));
		}

		rawResults = userDao.queryRaw(
			"select id,username from " + User.TABLE_NAME + " WHERE " + 
			"NOT EXISTS (select * from " + Project.TABLE_NAME + " where " + User.TABLE_NAME + "." + User.ID_FIELD_NAME + " = " + Project.TABLE_NAME + "." + Project.USER_ID_FIELD_NAME + ")",
			userDao.getRawRowMapper());

		for (User user : rawResults) {
			userList.put(user.getId(), new UserListItem(user.getUsername(), false));
		}
	}
